...
The build_aomp.sh script will install into $HOME/rocm/aomp_20.0-0 and create a symbolic link from $HOME/rocm/aomp to $HOME/rocm/aomp_20.0-0.
This feature allows multiple versions of AOMP to be installed concurrently.
To enable a backlevel version of AOMP, simply set AOMP to $HOME/rocm/aomp_19.0-3.
...
```
   wget https://github.com/ROCm-Developer-Tools/aomp/blob/aomp-20.0-0/bin/package.py
   spack create -n aomp -t makefile --force https://github.com/ROCm-Developer-Tools/aomp/releases/download/rel_20.0-0/aomp-20.0-0.tar.gz
   spack edit aomp
   spack install aomp
```
